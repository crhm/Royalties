...
/**The dialog to add a royalty for one channel only. Asks user to select a channel. Otherwise very similar to other dialog.
 * See AddRoyaltyDialog.
 * @author crhm
 *
 */
@SuppressWarnings("serial")
public class AddRoyaltyDialogOneChannel extends AddRoyaltyDialog {

	public AddRoyaltyDialogOneChannel(Book b) {
		super();
		book = b;
		...
		//Making the data entry panels
		JPanel panelBoth = new JPanel(new GridLayout(2, 1, 0, 0));
		JPanel panelDataEntry = new JPanel(new GridLayout(3, 2, 0, 0));
		...
		//Making channel label
		lblChannel = new JLabel("Channel:");
		lblChannel.setHorizontalAlignment(SwingConstants.CENTER);
		panelDataEntry.add(lblChannel);

		//Making the comboBox of channels
		String[] listChannels = new String[SalesHistory.get().getListChannels().size()];
		int countCH = 0;
		for (Channel ch : SalesHistory.get().getListChannels()) {
			listChannels[countCH] = ch.getName();
			countCH++;
		}
		Arrays.sort(listChannels);
		DefaultComboBoxModel<String> modelChannels = new DefaultComboBoxModel<String>(listChannels);
		cbbChannels = new JComboBox<String>();
		cbbChannels.setModel(modelChannels);
		cbbChannels.setSelectedIndex(-1);
		panelDataEntry.add(cbbChannels);
	 ...
	 * <br>Checks that a channel is selected.
	 * <br>Checks that an amount has been entered, and that it can be parsed as a double.
	 */
	protected void addFixedAmount() {
		if (cbbChannels.getSelectedIndex() == -1) {
			JOptionPane.showMessageDialog(this, "Error: Please select a channel for which to apply this royalty.", 
