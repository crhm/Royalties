...
/**Class intended to create/display a dialog for person creation
 * 
 * @author crhm
 *
 */
...
public class AddPersonDialog extends JFrame implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == bttnCancel) { //User cancels; window closes.
			this.dispose();
		} else if (e.getSource() == bttnCreate) { //User confirms - a new person is created
			...
			Person newPerson = ObjectFactory.createPerson(name.trim());
			...
			if (otherNames.contains(",") ) {
				...
				for (String s : otherNamesSeparated) {
					newPerson.addName(s.trim());
				}
			} else if (otherNames != null && !otherNames.isEmpty()) {
				newPerson.addName(otherNames.trim());
			}
